...
test('create index and update schema', async t => {
  ...
  const index = await catalog.openOrCreate('index', schema1)
  const tantivySchema1 = await index.getSchema()
  t.deepEqual(tantivySchema1, schema1, 'schema1 matches')
  await catalog.update('index', schema2)
  const tantivySchema2 = await index.getSchema()
  t.deepEqual(tantivySchema2, schema2, 'updated schema2 matches')
  catalog.close()
  await cleanup()
  t.end()
})
